...
5. `paper-repro-skill-source-policy`で利用元を確認し、依頼に該当する場合だけ
   ...
   `paper-repro-commit-output`、`paper-repro-delegate-integration`、`paper-repro-requirement-deferral`、`paper-repro-upstream-isolation`を読む。
...
作業の区切りごとにコミットする。区切りとは、**それ単独で意味が通り、
差し戻しても他に影響しない単位**である。
...
| 5択1件の作成、または選択結果の記録 |
| スキル1件の追加・改名 |
| 正本（`requirements.md` など）の変更 |
| 下流文書の追随 |

⚠️ **正本の変更と下流文書の追随は、別のコミットにする**
（[`docs/requirements-update-workflow.md`](docs/requirements-update-workflow.md) 3.3節）。
同じコミットに混ぜると、正本の変更だけを差し戻せなくなる。

⚠️ **統合や一括変換が済んだら、次の作業に入る前にコミットする。**
未コミットのまま次の編集を始めると、事故が起きたとき両方を失う。
2026年9月3日、改行コードを壊して `git checkout` で差し戻したとき、
未コミットだった `REQ-C03` の展開も同時に失った。
...
全コミット対象ファイル、履歴のURLを、**このチャットで1 URLずつ独立した`text`コードブロック**として出力する。
ターミナルへURLを表示しただけでは完了とせず、各コードブロックのコピーアイコンから個別にコピーできる形にする。
