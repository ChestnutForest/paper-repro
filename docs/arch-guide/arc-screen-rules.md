...
| フッタ | **謝辞（`CR-12`）**、出典表記、リポジトリへのリンク | 全画面で固定 |
...
## CR-12 外部サービスの謝辞（`paper-repro` 固有）

**外部サービスが製品への表示を求める文言は、フッタへ常時置く。**
...
| 規則 | 内容 |
|---|---|
| 表示 | **全画面のフッタに常時**。個別画面へ追加しない |
| 言語 | **原文のまま。翻訳しない**（`CR-09` の例外） |
| 省略 | **第2文を省かない** |
| ロゴ | **使わない**（下記） |

**第2文を必須とした理由。** 「arXiv の審査も承認も受けておらず、その見解を表すものでもない」
という否認である。第1文だけでは、arXiv が関与しているように読まれうる。
論文を読解・再現する製品では、この誤解が問題になる。

**翻訳しない理由。** 否認の効力は原文にある。`CR-09` は画面の文言を翻訳すると定めるが、
本文言はその例外とする。

### ロゴと名称

**arXiv のロゴ・配色を使わない。名称をブランドとして扱わない。**

API利用だけでは通常ロゴ使用は許諾されない。使わないことで、arXiv が定める
「独立プロジェクト」の条件を満たし続けられる。

⚠️ **出典の記述は制限の対象外である。** 「arXiv から取得」「arXiv URL を入力」といった
情報の記述は、ブランド利用にあたらない。製品名やロゴ的な扱いをしないという意味である。

### 成果物への表示

**レポートや zip にも同じ文言を含める。** 製品が生成して利用者が持ち出すものであり、
arXiv 由来のデータを含むためである。

### 他の外部サービス

GitHub、Hugging Face、OpenReview に同様の要求があるかは**未確認**である。
判明したら本節へ追加する。

根拠は [`../worknotes/decision-arxiv-acknowledgement.md`](../worknotes/decision-arxiv-acknowledgement.md) を参照。
...
- **謝辞（`CR-12`）のフッタ内での具体的な配置。** 文字サイズと、他の出典表記との並び。
- **成果物への謝辞の入れ方。** レポートの末尾か、別ファイルか。
