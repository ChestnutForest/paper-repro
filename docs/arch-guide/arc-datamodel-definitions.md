...
- 対象プロダクト: `paper-repro`
- 版: **v0.3**（2026年9月8日、要求5件の属性を追加）
- 作成日: 2026年8月29日
- 更新日: 2026年9月8日（`DM-E05`・`DM-E09`・`DM-E10` へ属性を追加）
- 前の更新: 2026年8月29日
...
| 状態・承認参照 | 草案／確定と承認を関連付ける | **確定** | `REQ-C06-R3.100`〜`.120`（承認の日時・対象・承認者・示した内容を記録し、`DM-E13` へ保存。訂正は新しい履歴として残す） |
...
| 状態・承認参照 | 解決、保留、採用理由 | **確定** | `REQ-C05-S01.90`〜`.110`（採用は利用者が決め、版整合・証拠・承認理由を保存。訂正の履歴を保持） |
| 変更した設定・変更理由 | 論文設定から変えた項目を1件も暗黙にしない | 確定 | `REQ-C05-S03.10`, `.30` |
| 原因候補の参照 | 再現結果が論文と食い違ったときに参照する対象 | 確定 | `REQ-C05-S03.70` |
| 不明の扱い | 推測で埋めず不明のまま残す | 確定 | `REQ-C05-S03.40` |
...
| 情報源の種別 | 公式実装、査読、採択、引用、著者実績、専門家意見、重要文献、データセット、二次解説、**ウェブ上の議論、照会による回答** | 確定 | `REQ-C07-R1.20` |
| 権利上の制約 | 補完源に付された再公開の制約。**制約なしと不明を区別する** | 確定 | `REQ-C07-S02.70`, `.80` |
| 発言者の確認状態 | 著者本人と確認できたか。**確認できない場合は確認済みとしない** | 確定 | `REQ-C07-S03.20`〜`.40` |
| 反応量 | 記録してもよいが**正しさの指標として提示しない** | 確定 | `REQ-C07-S03.70`, `.80` |
| 貢献の役割 | CRediT の役割語彙。人間と生成AIを区別する | 確定 | `REQ-C08-S01.10`, `.20` |
| 生成AIの利用記録 | 利用箇所、モデル名、日付。**著者・貢献者として登録しない** | 確定 | `REQ-C08-S01.50`〜`.70` |
...
| 乱数種・試行回数 | 同じ条件で再実行するために保存する | 確定 | `REQ-C05-S02.10`, `.60` |
| 要約統計 | 複数試行の結果。**単一実行と区別する** | 確定 | `REQ-C05-S02.50` |
| 試行の結末 | 成功、失敗、中断を区別する。**失敗と中断も残す** | 確定 | `REQ-C05-S02` 受入基準 |
...
| state | 未解決、解決、保留等 | 未決 | `REQ-C11` は段階2で**仕様が第2フェーズ送り**（USDM 第12章）。展開後に確定する |
