...
- **`IF-06` の接続先。** 初期リリース対象外のため未定。
  ⚠️ **`REQ-C09-S05`（外部照会）は初期リリースの範囲であり、不一致がある**
  （[`arc-interface.md`](arc-interface.md) 4.5.1節）。
- **保留**：サンドボックスからの外部アクセス
  - 担当工程：実装、次いでテスト
  - 解除条件：再現実装のサニティチェックで、検証コードが外部へアクセスする必要が生じるかを実測すること。
    `BR-12` は「ネットワークは原則遮断」と定めており、**遮断で足りるなら本書の範囲外である**
  - 再判断の期限：`REQ-C05-R2`（サニティチェック）を実装した時点
  - 保留の理由：**遮断で足りるかは、実際に再現実装を動かさないと分からない**
