...
| [`../.agents/skills/paper-repro-requirement-deferral/SKILL.md`](../.agents/skills/paper-repro-requirement-deferral/SKILL.md) | `paper-repro-requirement-deferral`。要求項目を今決めるか後工程へ送るかの判定と、保留の記録形式 |
| [`../.agents/skills/paper-repro-upstream-isolation/SKILL.md`](../.agents/skills/paper-repro-upstream-isolation/SKILL.md) | `paper-repro-upstream-isolation`。上流工程の途中で実装の状態を判断材料にしない規律 |
...
| [`requirements-decisions/batch-07-options.md`](requirements-decisions/batch-07-options.md) | **文書構造の第7バッチ**。既存サブ要求（`Sxx`）と USDM 下位要求（`Rn`）の関係を決める5択。**USDM 全要求展開の前提**。**選択肢3で確定**（2026-09-03） |
