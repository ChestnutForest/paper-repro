...
| [`pending-dm-e18-propagation.md`](pending-dm-e18-propagation.md) | 未整備事項の記録。`DM-E18 LearningProfile` のER図・CRUD図・論理属性定義・業務一覧への追随。**設計判断を3つ含む** | `REQ-C02-S01` |
| [`decision-material-requirements-source.md`](decision-material-requirements-source.md) | 判断材料の整理。要求文の正本を `requirements.md` と `requirements-usdm.md` のどちらにするか。**判断は行っていない** | — |
| [`workflow-model-three-candidates.md`](workflow-model-three-candidates.md) | 工程モデル3案（Phase 0〜6／読解順序／Seven Pillars）の階層の違いを整理した5択の前提。**採否は決めていない** | `ARF-M-01`、`PR-M-02` |
