...
## 2026-08-29：データモデル外部設計の具体化

| 指示 | 対応・成果物 |
|---|---|
| USDMで整理した要求全体からデータモデルを設計したい | 確定要求23件に現れる17エンティティを一覧化し、4領域のER図と論理属性定義を作成 |
| IPAのデータモデル編2冊と統合したい | ER図、エンティティ一覧、エンティティ定義、CRUD図の4成果物を正規の外部設計枠組みとして分離 |
| システム振舞いとの抜けを検出したい | 47業務をCRUD図で検査し、注釈・メモ群と演習履歴群の2件をUSDM再検討ギャップとして記録 |
| 既存の物理仕様を維持したい | 17エンティティの論理設計と、フェーズ0で実装対象のProject・Paper物理仕様を分離し、実装フェーズ0は変更しない |
| 次回以降も同じ手順で検証したい | `paper-repro-arch-guide`へ4成果物の作成順、論理・物理・実装状態の分離、Mermaid実描画を追加 |
| 枠組みを使って確定要求全体を再検査したい | 確定要求23件をデータ設計へ逆引きし、23/23の対応先と未解決ギャップ2件を一覧化 |
| PDFの観点で設計成果物を再検査したい | 14レビュー項目を実施し、✅2・🔨11・⏳1、全体成熟度「仕掛」と記録 |
| 要求書とER図の不一致を直したい | 取り込み前のProjectを表せるよう、Project 1件に対してPaper 0..1件へ多重度を統一 |
