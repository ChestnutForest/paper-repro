...
**未採用。** `paper-repro` の要求は `REQ-Cxx` 形式で、要求文と受入基準の対で管理しており
（[`requirements.md`](requirements.md) 第3章）、USDM の5要素階層は用いていない。

採用を検討する場合は、[`requirements-update-workflow.md`](requirements-update-workflow.md)
の手順に従い、要求候補として5択へ進めること。
