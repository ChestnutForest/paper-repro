...
`paper-repro-mvp` で使っている技術を、層ごとにまとめる。
「なぜそれを選んだか」も併記しているので、設計判断を振り返るときにも使う。

関連文書：[`mvp-design.md`](mvp-design.md)（設計）／[`roadmap.md`](roadmap.md)（導入時期）
...
**MVP の規模には十分**で、枯れており情報が豊富なため詰まったときに調べやすい、という理由で選定。
