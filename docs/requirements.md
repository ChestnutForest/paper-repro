...
- USDM 形式の要求仕様書: [`requirements-usdm.md`](requirements-usdm.md)（`REQ-C01`〜`REQ-C11` の11件を仕様まで展開済み。**`REQ-C12` と新規サブ要求6件は未展開。** 本書は要求文・受入基準・実装単位との対応を持つ）
...
| **REQ-C07-S02** | システムは、利用者が一次資料だけでは埋められない理解のギャップを記録し、そのギャップを埋めるために用いた補完源を、種類、主体、取得日時および権利上の制約とともに保持するものとする。補完源に付された権利上の制約に反する再公開を行わないものとする。未解決のまま残るギャップを、解消済みのものと区別して保持するものとする | 理解のギャップを対象箇所へ関連付けて記録でき、補完源の種類、主体、取得日時、権利上の制約を保持できる。制約に反する再公開を行わない。未解決のギャップを解消済みと区別できる | 未割当（第8バッチで新設） |
| **REQ-C07-S03** | システムは、公開の場での議論から得た情報について、発言者の属性が確認済みか未確認かを区別し、対象の論文箇所へ紐づけて保持するものとする。反応の量を正しさの根拠として扱わないものとする。特定の外部サービスが利用不能になった場合でも、取得済みの記録を参照できるものとする。利用規約に反する自動収集を行わないものとする | 発言者が著者本人であることが確認できない場合、確認済みとして表示しない。議論を対象の論文箇所へ紐づけられる。反応量を正しさの指標として提示しない。外部サービスが利用不能でも取得済みの記録を参照できる。利用規約に反する自動収集を行わない | 未割当（第8バッチで新設） |
| **REQ-C08-S01** | システムは、再現プロジェクトにおける貢献をCRediTの役割語彙で記録し、生成AIが担った部分を人間の貢献と区別して保持するものとする。生成AIを著者または貢献者として登録できないものとし、あくまで使用の記録として扱うものとする。再現性の検証（Validation）を独立した役割として明示するものとする | 貢献を役割ごとに記録でき、生成AIの利用箇所・モデル名・日付を保持できる。生成AIを著者・貢献者として登録できない。Validationを独立した役割として区別できる。記録を成果物に添付できる定型で出力できる | 未割当（第11バッチで具体化） |
...
- **保留**：第9章の対応表と [`product-design.md`](product-design.md)・[`roadmap.md`](roadmap.md) の突き合わせ
  - 担当工程：要件定義（本サイクル内で決められる）
  - 解除条件：第9章の対応表33行それぞれについて、対応する実装単位が
    `product-design.md` の画面・API の記述と `roadmap.md` のフェーズ計画に現れるかを確認すること
