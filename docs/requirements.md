...
- 版: **v0.3.1**（`REQ-C02-S01` の具体化。利用者の状態の横断保持）
- 前版: v0.3（工程モデルの確定とサブ要求2件の追加）／ v0.2.1（工程モデルの確定）／ v0.2（選択済み要求の統合完了）／ v0.2-draft（骨組み更新）／ v0.1（初版・ドラフト）
...
> **本版で確定したもの。** 選択済みの `REQ-C01`〜`REQ-C11` と、具体化済みサブ要求15件を、
> 要求文・受入基準つきで第3章へ統合した。第4〜5章の実装単位は、この要求へ対応づけて更新した。
> v0.3 では**工程モデルの確定**（第1.1節）と、**再現の妥当性に関するサブ要求2件**
> （`REQ-C05-S02`・`REQ-C05-S03`）を追加し、第0章の判定基準と第8章の受け入れ基準を改めた。
> v0.3.1 では **`REQ-C02-S01` を具体化**し、既読資料と専門分野をプロジェクト横断で保持することを定めた。
>
> **本版で確定していないもの。** 新しい一次資料3件（`simclr-handson-deck.md`、
> `academic-research-skills.md`、`academic-research-skills-frameworks.md`）および
> `ml-research-practice-advice.md` から抽出した要求候補のうち、5択を経ていないものは本書に含めない。
> 突合の結果は [`requirements-analysis/`](requirements-analysis/) の `crosswalk-*.md` にある。
> 残る新規メイン要求候補は `PR-M-09`・`PR-M-13`・`ARF-M-08`・`ARF-M-12` の4件で、次の5択で扱う。
...
| サブ要求 | `REQ-C02-S01` ほか | 15 | メイン要求を具体化する一次識別子 |
...
**判定は複数回の実行の平均に対して行う**（2026年9月2日、[`requirements-decisions/batch-05-options.md`](requirements-decisions/batch-05-options.md) 選択肢3）。
各回すべてが範囲に収まることは求めない。ばらつきの大きい指標で不必要に厳しくなるためである。
**ただし試行回数と標準偏差を併記することを条件とする。** 単一実行の結果は「単一実行」と明示する。
...
> **Phase 0〜6 を工程モデルとして確定する**（2026年9月2日、[`requirements-decisions/batch-04-options.md`](requirements-decisions/batch-04-options.md) 選択肢4）。
> 第1パスの内部手順として、Title → 著者 → Abstract → 図表 → SI の読解順序を定める。
> あわせて、論文の取り込み後に **Scope（この論文を読むのに何が足りないかの評価）** を段階として置き、
> 結果を `REQ-C02`（前提知識の段階的説明）へ渡す。
>
> **Scope の結果は `REQ-C02-S01` によりプロジェクト横断で保持する**（2026年9月2日、[`requirements-decisions/batch-06-options.md`](requirements-decisions/batch-06-options.md) 選択肢3）。
> ⚠️ **利用者の熟達段階の保持と推定（`ARF-M-12`）は行わない。** 保持するのは「読んだ／受講した」という事実であり、理解度の判定ではない。
...
|---|---|---|---|
| **REQ-C02-S01** | システムは利用者が既に読んだ論文および学習した資料をプロジェクト横断で保持し、対象論文の前提知識と突き合わせて、既習の項目の説明を省けるようにするものとする。前提知識が不足したまま実装へ進もうとした場合に注意を出す。利用者の専門分野を設定として保持し、説明の例示と転用先の提案をその分野に寄せる。**熟達段階の推定は行わない** | 既読・既学習の資料をプロジェクトを越えて参照でき、重複する説明を省ける。不足のまま実装へ進もうとしたときに注意が出る。専門分野を設定でき、説明の例示に反映される。保持するのは読んだ・受講したという事実に限り、理解度をLLMが判定しない | `F-16`、`B-04`、`B-09` |
...
| **REQ-C05-S02** | システムは再現実行について、乱数種、試行回数、各試行の結果および要約統計を記録し、単一実行の結果を「単一実行」と明示するものとする。失敗した試行および途中で止めた試行も記録に残す | 成果物に載せる数値へ試行回数を併記でき、同じ乱数種で再実行できる。最良値だけを取り出す操作に注意を出し、単一実行を複数実行と区別できる | `B-06`、`B-08`、`F-08`、`F-10` |
| **REQ-C05-S03** | システムは論文設定から変更した設定を差分Δとして残し、各行に変更点、変更理由、期待される性能差および出典を持たせ、仮定台帳へ転記するものとする。性能差を断定できない場合は不明と記録し、推測値を書かない | 変更した設定を1件も暗黙にせず表に残せる。不明を推測で埋めず、再現結果が論文と食い違ったときに差分Δの各行を原因候補として参照できる | `B-07`、`F-06`、`F-07` |
...
次の1件は、対応する一次資料の小節がまだ分析されておらず、要求文を具体化していない。
...
| F-02 | パイプライン・ウィザード | Phase 0〜6 を横断するステッパー。現在地・完了・保留を可視化。各 Phase を行き来できる。**読解・学習と再現実装の2経路、途中切替、段階的な成果物を表示する。第1パスの内部手順と Scope 段階を含む（第1.1節参照）** | `REQ-C01` |
...
| F-10 | スコア照合ビュー | 再現値 vs 論文値を並べ、差分と ±5 判定を色分け。サチュレーション側/崩壊側タスクを明示。**試行回数・標準偏差・差分Δを併記し、単一実行はそう明示する** | `REQ-C05` |
...
| B-08 | スコア照合 | 再現値と論文値（表から抽出）を突き合わせ、±5 判定。平均の取り方（単純／重み付き）を検算。**複数回実行の平均と標準偏差を算出する** | `REQ-C05` |
...
識別力の高い2タスクで**複数回実行の平均が**論文値と ±5 に収まる Colab ノートブックを生成・ダウンロードできる。**試行回数と標準偏差を併記し、論文設定からの変更点を差分Δとして残す**」。
...
⚠️ **5件を検証した**（2026年9月7日）。**3件は既に決着しており、未決事項ではなかった。**
残る2件は後工程で決める（[`../.agents/skills/paper-repro-requirement-deferral/SKILL.md`](../.agents/skills/paper-repro-requirement-deferral/SKILL.md)）。

#### 決着済み（記載が古かったもの）

いずれも [`product-design.md`](product-design.md) 第4.2節の技術スタック表が答えを持つ。

| 項目 | 決着の内容 |
|---|---|
| ~~認証・マルチユーザー~~ | **初期リリースは単一ユーザー + Bearer トークン。** OAuth は将来（同 第178行・第4.2節） |
| ~~LLM プロバイダの固定と切替~~ | **Claude API を抽象化層で包む。** 他プロバイダへ差し替え可能な形にし、プロバイダ非依存とする |
| ~~実行基盤~~ | **初期リリースは Docker Compose、拡張時に k8s。** サンドボックスは gVisor もしくは Firecracker 上の使い捨てコンテナ |

⚠️ **下流文書が先に決まり、正本の未決事項が更新されていなかった。**
[`roadmap.md`](roadmap.md) と実装の間にも同種のずれがある。

#### 後工程で決めるもの

- **保留**：論文の figure キャプチャ（自動抽出か手動貼り付けか）
  - 担当工程：データモデル編の設計、次いで実装
  - 解除条件：[`arch-guide/arc-datamodel-definitions.md`](arch-guide/arc-datamodel-definitions.md) に
    論文の図表を保持するエンティティの属性が確定し、PDF からの抽出精度を実測で確かめること
  - 再判断の期限：データモデル編の定義が完成し、`Paper` の物理仕様が確定した時点
  - 保留の理由：自動抽出の可否は PDF の構造に依存し、**取り込みを実装して測らないと判断できない。**
    手動貼り付けを前提に属性を決めると、後から自動抽出を足すときに構造を変えることになる。

- **保留**：オフラインとセルフホストの要件
  - 担当工程：外部インタフェース編の設計、次いで実装
  - 解除条件：[`arch-guide/arc-interface-list.md`](arch-guide/arc-interface-list.md) の6件について、
    外部依存を切った場合に何が動かなくなるかが列挙されること
  - 再判断の期限：外部インタフェース編の一覧と対応表が完成した時点
  - 保留の理由：`REQ-C09-S01` は「特定端末・サービスを必須にしない」と定めるが、
    論文の取得と LLM の呼び出しは外部に依存する。**どこまで切れるかは依存の一覧を作らないと分からない。**

⚠️ **2件とも根拠がある。** 前者は `REQ-C03`（論文の取り込み）、
後者は `REQ-C09-S01.160`・`.170`（端末とサービスへの非依存）が根拠である。
根拠のない候補は保留にせず取り下げる（`REQ-C09-S04` の例）。

### 10.2 本版で新たに生じたもの

- ~~**工程モデルの確定**（第1.1節）~~ → **解決済み**（2026年9月2日）。
  [`requirements-decisions/batch-04-options.md`](requirements-decisions/batch-04-options.md) の選択肢4により、
  Phase 0〜6 を確定し、第1パスの読解順序と Scope 段階を取り込んだ。
