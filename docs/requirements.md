...
- USDM 形式の要求仕様書: [`requirements-usdm.md`](requirements-usdm.md)（**メイン要求12件・サブ要求21件のすべてを仕様まで展開済み**、2026年9月8日。本書は要求文・受入基準・実装単位との対応を持つ）

> **本版で確定したもの。** 選択済みの `REQ-C01`〜`REQ-C12` と、具体化済みサブ要求21件を、
...
> ⚠️ **新規メイン要求の候補は打ち止めである**（2026年9月7日、
> [`requirements-analysis/README.md`](requirements-analysis/README.md) の突合結果）。
> `ARF-M-12` は第6バッチ、`PR-M-09`・`PR-M-13`・`ARF-M-08` は第11バッチで決着した。
>
> **v0.4 で確定したもの**（2026年9月7日）。第8〜11バッチの承認内容を反映した。
> `REQ-C12`（再現スコープの定義）を新設し、サブ要求6件
> （`REQ-C04-S03`・`REQ-C07-S02`・`REQ-C07-S03`・`REQ-C08-S01`・`REQ-C09-S05`・`REQ-C10-S05`）を加えた。
> `REQ-C08-S01` の具体化により、**未具体のサブ要求は無くなった。**
> `PR-M-13`（設計原理の転用）は第2フェーズへ送った
> （[`worknotes/pending-crosswalk-deferred.md`](worknotes/pending-crosswalk-deferred.md) 2.5節）。
...
| 種別 | 記法 | 件数 | 役割 |
|---|---|---|---|
| メイン要求 | `REQ-C01`〜`REQ-C12` | 12 | 要求の一次識別子 |
| サブ要求 | `REQ-C02-S01` ほか | 21 | メイン要求を具体化する一次識別子 |
| 実装単位（FE） | `F-01`〜`F-21` | 21 | 画面・機能の粒度 |
| 実装単位（BE） | `B-01`〜`B-16` | 16 | サービス・API の粒度 |
...
システムは論文から今回再現する要素を4〜6個に絞ったスコープ定義書を生成し、
データセット、モデル、損失、評価の4区分を必ず含め、スコープ外とした要素を除外理由とともに
明記するものとする。各構成要素には実装の完了判定条件を紐づける。

- **受入基準**: 4区分を含む定義書を生成でき、スコープ外の要素を除外理由つきで列挙でき、
  各構成要素に完了判定条件（サニティチェック）が紐づく。定義書を後段の実装依頼の入力として
  そのまま使える粒度で出力できる。
- **実装単位**: `F-21`、`B-16`
- **由来**: `PR-M-09`（[`requirements-decisions/batch-11-options.md`](requirements-decisions/batch-11-options.md) 選択肢1）

⚠️ **`REQ-C05`（再現実装の支援範囲）とは別の要求である。**
`REQ-C05` の要求文は読み解き・生成・実行・通し・照合を**支援範囲とする**と限定しており、
**スコープの決定はその外側にある。**
...
| **REQ-C04-S03** | システムは、利用者が論文の記述、数式または実装について、生成AIとの対話を通じて理解を深められる場を提供するものとする。対話は、論文の特定箇所へ関連付けて開始でき、その箇所の原文、周辺の文脈および利用者が既に記録した理解状態を対話の入力に含められるものとする。対話の過程で埋められた導出のギャップ、確認された前提、および解消されなかった疑問は、`REQ-C04-S02`が定める自己説明および未理解点として記録できるものとする。対話の成立を理解の達成として自動判定せず、理解状態の確定は利用者の操作によるものとする | 対話を論文の節、数式、図表または実装箇所へ関連付けて開始でき、対象箇所の原文と周辺の文脈を入力に含められる。数式を含む入力を扱える。対話の過程を保存し後から参照できる。埋められた導出のギャップを対象へ関連付けて記録でき、解消されなかった疑問を`REQ-C04-S02`の記録へ引き継げる。対話の完了をもって理解状態を自動的に理解済みへ変更しない。特定のモデル名または提供事業者に依存しない形で記録を保持できる。利用者が対話を用いずに読解工程を進めることを妨げない | `F-20`、`B-15` |
...
| **REQ-C07-S02** | システムは、利用者が一次資料だけでは埋められない理解のギャップを記録し、そのギャップを埋めるために用いた補完源を、種類、主体、取得日時および権利上の制約とともに保持するものとする。補完源に付された権利上の制約に反する再公開を行わないものとする。未解決のまま残るギャップを、解消済みのものと区別して保持するものとする | 理解のギャップを対象箇所へ関連付けて記録でき、補完源の種類、主体、取得日時、権利上の制約を保持できる。制約に反する再公開を行わない。未解決のギャップを解消済みと区別できる | `F-17`、`B-02` |
| **REQ-C07-S03** | システムは、公開の場での議論から得た情報について、発言者の属性が確認済みか未確認かを区別し、対象の論文箇所へ紐づけて保持するものとする。反応の量を正しさの根拠として扱わないものとする。特定の外部サービスが利用不能になった場合でも、取得済みの記録を参照できるものとする。利用規約に反する自動収集を行わないものとする | 発言者が著者本人であることが確認できない場合、確認済みとして表示しない。議論を対象の論文箇所へ紐づけられる。反応量を正しさの指標として提示しない。外部サービスが利用不能でも取得済みの記録を参照できる。利用規約に反する自動収集を行わない | `F-17`、`B-02` |
| **REQ-C08-S01** | システムは、再現プロジェクトにおける貢献をCRediTの役割語彙で記録し、生成AIが担った部分を人間の貢献と区別して保持するものとする。生成AIを著者または貢献者として登録できないものとし、あくまで使用の記録として扱うものとする。再現性の検証（Validation）を独立した役割として明示するものとする | 貢献を役割ごとに記録でき、生成AIの利用箇所・モデル名・日付を保持できる。生成AIを著者・貢献者として登録できない。Validationを独立した役割として区別できる。記録を成果物に添付できる定型で出力できる | `F-17`、`B-13`、`N-06`、`N-09` |
...
| **REQ-C09-S05** | システムは、論文著者その他の外部関係者への照会について、送信の記録、応答の有無、応答内容および利用者が行った返礼を追跡できるようにするものとする。照会の送信は利用者の明示的な操作によるものとし、システムが自動的に外部へ照会を送信しないものとする。応答がない場合を失敗として扱わず、未応答の状態として保持するものとする。応答者に義務が生じないことを前提とし、応答率、応答速度その他の指標により応答者を評価しないものとする | 照会を対象論文、対象ギャップ、送信日時とともに記録できる。送信は利用者の明示的な操作による。未送信、送信済み、応答あり、未応答を区別できる。未応答を失敗、エラーまたは拒否として扱わない。返礼を対象の照会へ関連付けて記録できる。応答率等により個々の応答者を評価または序列化しない。連絡先情報を利用者の管理下で保持し目的外に利用しない。照会の文面を生成する場合、送信前に利用者が確認・編集できる | `F-20`、`B-15` |
...
| **REQ-C10-S05** | システムは、生成AIが出力した内容について、利用者が論理を追跡し、誤りを特定し、指摘を行い、訂正の経過を記録できるようにするものとする。生成内容は検証されていない候補として扱い、利用者が検証し採用を判断するまで、検証済みの結論へ昇格させないものとする。生成内容と、一次資料から確認された内容、および利用者が自ら検証した内容を、区別して保持するものとする。出力の流暢さ、断定的な表現または生成の完了を、正しさの根拠として扱わないものとする | 生成内容を対象の論文箇所、数式または主張へ関連付けて保存できる。一次資料由来および利用者が検証した内容と区別して表示できる。未検証、検証済み、誤りを含む、判断保留を区別できる。誤りの箇所と理由を記録でき、訂正の経過を保持できる。利用者の検証・採用判断なしに検証済みの結論へ昇格させない。流暢さ、断定的表現、生成完了を正しさの指標として提示しない。確からしさを単一の数値で提示しない。検証に用いた根拠を関連付けられる | `F-19`、`B-12` |

### 3.3 未具体化のサブ要求

⚠️ **未具体のサブ要求は無くなった**（2026年9月7日）。
`REQ-C08-S01` は第11バッチで `ARF-M-08`（貢献の切り分け）を中身として具体化し、
第3.2節へ移した（[`requirements-decisions/batch-11-options.md`](requirements-decisions/batch-11-options.md) 選択肢1）。
...
| F-06 | 仮定台帳テーブル | 曖昧箇所・論文の記述・選択・根拠・**疑わしさ(高/中/低)**・検証状況を表で編集。ソート・フィルタ可。**F-05 と同じ区分体系を用いる** | `REQ-C07`、`REQ-C08`、`REQ-C09-S01`、`REQ-C10-S01`〜`S04`、`REQ-C05-S03` |
| F-07 | Δリスト | 差分を1〜3個で管理。「5行を超えたら警告」（＝まだ理解不足のサイン） | —、`REQ-C05-S03` |
| F-08 | サニティ階段ビュー | タイプA/B別の各段を pass/fail バッジで表示。**オラクル満点・1バッチ過学習の段を強調** | `REQ-C05`、`REQ-C05-S02` |
| F-09 | 実行コンソール | サンドボックス実行のログ・stdout・生成画像を表示。中間ファイルを目視できる | `REQ-C05` |
| F-10 | スコア照合ビュー | 再現値 vs 論文値を並べ、差分と ±5 判定を色分け。サチュレーション側/崩壊側タスクを明示。**試行回数・標準偏差・差分Δを併記し、単一実行はそう明示する** | `REQ-C05`、`REQ-C05-S02` |
...
| F-16 | 段階的学習支援 | 用語、数式、機械学習の前提説明、操作可能な具体例・反例・手計算、確認問題、自己説明、根拠付きフィードバックを提供する | `REQ-C02`、`REQ-C04`、`REQ-C04-S01`〜`S02`、`REQ-C02-S01` |
| F-17 | 証拠・プロヴェナンス表示 | 主張ごとの区分、直接支持範囲、根拠、不一致、不確実性、実装の入手・検査状態、分類・訂正履歴、影響成果物を閲覧する。**補完源の種類・主体・権利上の制約、公開議論の発言者が確認済みか未確認かを区別して表示する。貢献の役割と生成AIの利用箇所を示す** | `REQ-C07`、`REQ-C07-S01`、`REQ-C08`、`REQ-C10-S04`、`REQ-C07-S02`〜`S03`、`REQ-C08-S01` |
| F-18 | 学習ポートフォリオ | 論文更新、学習履歴、重要文献の深掘りキュー、関連論文、引用・推薦・利用信号、探索状態、要約・索引、自己説明、公開範囲、再学習対象を表示・管理する | `REQ-C09`、`REQ-C09-S02`〜`S03` |
| F-19 | 批判的検証ワークスペース | 主張区分、直接支持範囲、前提、帰結、成立範囲、読解深度、実験条件、支持例・反例・境界条件、予想と観察、仮説、矛盾、誤り・誇張・バイアス等の検討状態、公式実装、再現結果を関連付け、人間が最終判断する。**生成内容を未検証・検証済み・誤りを含む・判断保留で区別し、一次資料由来と利用者が検証した内容と分けて示す** | `REQ-C10`、`REQ-C10-S01`〜`S04`、`REQ-C10-S05` |
| F-20 | 質問・対話・共同読解 | 個人質問台帳から生成AI対話、限定共有、専門家・著者問い合わせへ段階拡張し、回答主体と公開範囲を管理する。**対話を論文箇所へ関連付けて開始し、照会の未送信・送信済み・応答あり・未応答を区別する。未応答を失敗として扱わない** | `REQ-C11`、`REQ-C04-S03`、`REQ-C09-S05` |
| F-21 | 再現スコープ定義書 | 今回再現する要素を4〜6個に絞り、データセット・モデル・損失・評価の4区分で表示する。**スコープ外の要素を除外理由つきで並べ、各構成要素に完了判定条件を紐づける** | `REQ-C12` |
...
| B-02 | 実装探索 | GitHub 検索・Papers with Code・OpenReview を横断し、公式実装／査読コメントを収集。**公式実装、査読、採択、引用、著者実績、専門家意見、重要文献、データセット、二次解説を区別し、実装の版・取得可否・検査可否を真偽判定なしで証拠・深掘り台帳へ登録する** | `REQ-C05-S01`、`REQ-C07`、`REQ-C07-S01`、`REQ-C09-S02`、`REQ-C07-S02`〜`S03` |
...
| B-04 | LLMオーケストレータ | Claude API で：第1〜3パスの要約、spec 草案、数式→コード下訳、shape 導出検算、台帳の穴の洗い出し、翻訳。**出力は必ずサニティに通す前提。理論解説、具体例・反例・手計算課題、自己説明へのフィードバック、生成条件の記録を追加する** | `REQ-C02`、`REQ-C04`、`REQ-C04-S01`〜`S02`、`REQ-C08`、`REQ-C02-S01` |
| B-05 | サンドボックス実行 | 使い捨てコンテナで、公式コード／生成コード／サニティチェックを実行。★第7章★ **Pythonを初期対象とし、言語ごとの専用プロファイルを審査後に追加する** | `REQ-C05`、`REQ-C06` |
| B-06 | サニティ実行 | タイプA：shape→勾配→1バッチ過学習→ベースライン。タイプB：データ実測→抽出→**オラクル満点→異常系0点**→E2E→照合 | `REQ-C05`、`REQ-C05-S02` |
| B-07 | ノートブック生成 | Colab 用 `.ipynb` を生成（環境ピン留め・サニティ段・推論・照合を含む構成） | `REQ-C05`、`REQ-C05-S03` |
| B-08 | スコア照合 | 再現値と論文値（表から抽出）を突き合わせ、±5 判定。平均の取り方（単純／重み付き）を検算。**複数回実行の平均と標準偏差を算出する** | `REQ-C05`、`REQ-C05-S02` |
| B-09 | 台帳・PJ 永続化 | 仮定台帳・spec・Δ・承認履歴・成果物を DB／ストレージに保存。**論文ファイル、注釈、計算、メモ、読書位置、バックアップ、証拠、プロヴェナンス、学習履歴、同意情報を段階的に追加する** | `REQ-C07`〜`REQ-C09`、`REQ-C09-S01`、`REQ-C02-S01` |
| B-10 | 成果物パッケージ | 規約名 zip（JST）を生成。個別ファイルも提示可能に | — |
| B-11 | コスト計上 | LLM／実行の消費を Phase 単位で記録し、**予算上限で自動停止** | — |
| B-12 | 証拠台帳管理 | 情報源の種別・版・著者性・直接性・一致状況に加え、前提、帰結、成立範囲、実験条件、主張区分、検討状態と人間の採用根拠を保存する | `REQ-C07`、`REQ-C10`、`REQ-C10-S01`〜`S04`、`REQ-C10-S05` |
| B-13 | プロヴェナンス管理 | 原文から要約、解釈、分類、問題、条件変更、予想、計算、観察、反例、仮説、利用者訂正、要求、実装、テスト、承認までを、生成条件とともに双方向追跡する | `REQ-C08`、`REQ-C10-S03`〜`S04`、`REQ-C08-S01` |
| B-14 | 論文更新・学習履歴 | 論文・公式実装の更新差分、理解確認、重要文献、深掘りキュー、関連グラフ、時点付き参考信号、要約・索引、自己説明、公開範囲、知識再利用を管理する | `REQ-C09`、`REQ-C09-S02`〜`S03` |
| B-15 | 質問・回答管理 | 質問、回答、注釈、主体、情報源、媒体、原文、要約、対象箇所、変更履歴、公開範囲を保存する。**対話の過程、照会の状態と返礼、権利上の制約を保持する** | `REQ-C11`、`REQ-C04-S03`、`REQ-C09-S05` |
| B-16 | 再現スコープ管理 | スコープ定義書を生成し、4区分と除外理由、完了判定条件を保存する。**後段の実装依頼の入力として出力できる粒度で保持する** | `REQ-C12` |
...
**最終製品要求と初期実装範囲は別物である。** 第3章の12件は最終到達点であり、
...
- `F-07` Δリスト
- `F-08` サニティ階段、`F-09` 実行コンソール
- `F-10` スコア照合、`F-11` 成果物ビュー
- `F-12` 承認ゲート（ゲート①〜③と、④⑤⑥の最小形）
- `F-13` プロジェクト管理、`F-14` コスト表示
- `F-15` 利用経路選択（`REQ-C01` の最小実装）
- `F-17` 証拠・プロヴェナンス表示（**解決待ちパネルの最小形のみ**）
- `F-21` 再現スコープ定義書
- `B-01`〜`B-04` 取り込み・実装探索・リポジトリ読解・LLMオーケストレータ
- `B-05`/`B-06` サンドボックス（**CPU のみ・ネットワーク遮断**）＋ タイプB のサニティ階段
  （オラクル満点・異常系0点まで）
- `B-07`/`B-08` ノートブック生成・照合
- `B-09` 永続化、`B-10`/`B-11` パッケージ・コスト
- `B-12`〜`B-15`（**`pending` の最小形のみ**）
- `B-16` 再現スコープ管理
- `N-01`〜`N-05`、`N-07`
- `N-06` 監査ログ（**承認履歴のみ**）
...
- `F-18`〜`F-20`、`N-08`〜`N-10`（`REQ-C09`〜`REQ-C11` の段階2以降）→ 第2フェーズ以降
- `F-17` と `B-12`〜`B-15` の、上記の最小形を超える部分（`REQ-C08` の段階2以降）→ 第2フェーズ以降

**本節は [`product-design.md`](product-design.md) 第6章の初期リリース判定と一致させてある**
（2026年9月11日に突き合わせ。第10.2節の保留1件目の解除）。**判定を変えるときは両方を直す。**
...
| REQ-C02-S01 | F-16 | B-04、B-09 | — |
| REQ-C05-S02 | F-08、F-10 | B-06、B-08 | — |
| REQ-C05-S03 | F-06、F-07 | B-07 | — |
| REQ-C04-S03 | F-20 | B-15 | — |
| REQ-C07-S02 | F-17 | B-02 | — |
| REQ-C07-S03 | F-17 | B-02 | — |
| REQ-C08-S01 | F-17 | B-13 | N-06、N-09 |
| REQ-C09-S05 | F-20 | B-15 | — |
| REQ-C10-S05 | F-19 | B-12 | — |
...
- ~~**新しい一次資料3件の要求候補32件**の扱い。~~ → **解決済み**（2026年9月8日）。
  [`worknotes/pre-approval-screening.md`](worknotes/pre-approval-screening.md) は
  5択へ進めるべきものを5〜7件程度と見込んでいたが、**実際は3件だった。**

  | 内訳 | 件数 |
  |---|---|
  | クロスウォーク4本で現行要求へ吸収（✅ / 🔨） | 30 |
  | 5択へ進み確定（`PR-M-09`・`PR-M-13`・`ARF-M-08`、第11バッチ） | 3 |
  | 突合の結果、既存要求の具体化と判定（`ARF-M-13`・`PR-M-15`） | 2 |
  | 保留（担当工程と解除条件つき） | 4 |

  ⚠️ **新規メイン要求として5択へ送る候補は打ち止めである**
  （[`requirements-analysis/README.md`](requirements-analysis/README.md) の突合結果、
  [`requirements-analysis/crosswalk-method.md`](requirements-analysis/crosswalk-method.md) 6節）。
  保留4件は [`worknotes/pending-crosswalk-deferred.md`](worknotes/pending-crosswalk-deferred.md) にある。

- ~~`REQ-C02-S01` の具体化~~ → **解決済み**（2026年9月2日、第6バッチ選択肢3）。
  `REQ-C09-S04` は取り下げ済み（2026年9月7日）。
  ~~`REQ-C08-S01` の具体化~~ → **解決済み**（2026年9月8日、第11バッチ選択肢1）。
  **未具体のサブ要求は無くなった。**

- ~~**保留**：第9章の対応表と [`product-design.md`](product-design.md) の突き合わせ~~
  → **解除**（2026年9月11日）

  **実測の結果。** 第9章の対応表33行が参照する実装単位は
  `F-01`・`F-02`・`F-05`〜`F-10`・`F-12`・`F-13`・`F-15`〜`F-21`、`B-01`〜`B-16`、
  `N-06`・`N-08`〜`N-10` である。このうち [`product-design.md`](product-design.md) 第6章に
  行が無かったのは **`F-21` と `B-16` の2件だけ**で、残りはすべて初期リリースの判定を持っていた。
  2件は `REQ-C12`（再現スコープの定義）の実装単位であり、同章が v0.3（2026年9月3日）で
  止まっていたために欠けていた。**2件を「含む」と判定し、第6章へ行を追加した。**

  判定の理由：`REQ-C12` は各構成要素へ完了判定条件（サニティチェック）を紐づける要求であり、
  初期リリースの受け入れ基準にある「オラクル満点確認」と「識別力の高い2タスク」の選定が
  その出力に依存する。**先送りすると、何を再現するかが決まらないまま spec を書く形になる。**

  第8章の2つの一覧は、第6章の判定と一致するよう全面的に書き直した。
  **書き直し前は、第9章が参照する実装単位のうち21件が第8章のどちらの一覧にも現れず、
  `F-17` と `B-12`〜`B-15` は第6章が「最小形のみ」とするのに対し第8章は先送り側に置いていた。**

  ⚠️ **2026年9月11日、解除条件から [`roadmap.md`](roadmap.md) を外したうえで解除した。**
  実測したところ、`roadmap.md` に実装単位ID（`F-xx`・`B-xx`・`N-xx`）は1件も現れず、
  要求IDも `REQ-C01` の1件だけだった。同書はフェーズ名と確認画面で記述する方針を明示しており、
  **ID照合では解除条件を判定できない。** 要求から実装単位への対応は第9章が、
  実装単位から設計箇所への対応は `product-design.md` 第6章が持つため、
  `roadmap.md` へ同じ対応を持たせると3箇所目の重複になる。

- **保留**：`F-17` と `B-12`〜`B-15` の「最小形」が、2026年9月8日に加わったサブ要求で足りるか
  - 担当工程：`F-17` は画面編の設計、`B-12`〜`B-15` は外部インタフェース編の設計
  - 解除条件：`REQ-C07-S02`・`REQ-C07-S03`（補完源の権利上の制約、未解決ギャップの区別）が
    `F-17` の解決待ちパネルの最小形に収まるか、`REQ-C04-S03`・`REQ-C09-S05`・`REQ-C10-S05` が
    `B-12`〜`B-15` の `pending` の最小形に収まるかを、表示項目と保持項目を書き出して判断できること
  - 再判断の期限：`S-04-01`（リーディング作業台）の入出力項目一覧を書く時点
  - 保留の理由：**初期リリースに含めるかどうかは第8章で決着しており、残るのは最小形の中身である。**
    中身は画面と外部接点の設計で決まるため、要件定義では決められない

- **保留**：非機能要求 `N-01`〜`N-10` を要求として起こすか
  - 担当工程：要件定義（本サイクル内で決められる）
  - 解除条件：`N-01`〜`N-10` のうち対応要求を持たない6件（`N-01`〜`N-05`、`N-07`）について、
    要求文と受入基準を書けるだけの根拠が一次資料にあるかを確認すること
  - 再判断の期限：USDM への展開が完了した時点
  - 保留の理由：REF-17 2.2.9 は非機能要求も要求仕様書へ含めるとするが、
    ⚠️ **本書の `N-01`〜`N-10` は要求ではなく実装単位である**（ID体系の表を参照）。
    `F-xx`・`B-xx` と同じ列に置かれ、10件中6件は対応要求を持たない。
    **「書き直す」のではなく「新しく要求を起こす」作業になるため、規模の見積もりが要る**
    （[`requirements-usdm.md`](requirements-usdm.md) 第13章）
