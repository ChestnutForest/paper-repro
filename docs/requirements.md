...
- **保留**：第9章の対応表と [`product-design.md`](product-design.md) の突き合わせ
  ...
  - 解除条件：第9章の対応表33行それぞれについて、対応する実装単位が
    `product-design.md` 第6章「要求と設計の対応表」に現れて初期リリースの判定を持つことを確認し、
    第8章「初期リリース範囲」の2つの一覧をその判定と一致させること
  - 再判断の期限：USDM への展開が完了した時点
  - 保留の理由：**後工程を待つ必要はない。確認作業そのものが未着手である。**
    ただし第8〜11バッチの反映で対応表が33行へ増えたばかりであり、
    USDM 展開で仕様が確定してから照合するほうが、同じ作業を二度行わずに済む

  ⚠️ **2026年9月11日、解除条件から [`roadmap.md`](roadmap.md) を外した。**
  ...
  **ID照合では解除条件を判定できない。** 要求から実装単位への対応は第9章が、
  ...
  ⚠️ **照合先の `product-design.md` は v0.3（2026年9月3日06時49分）で止まっている。**
  以後 `REQ-C04-S03`・`REQ-C07-S02`・`REQ-C07-S03`・`REQ-C09-S05`・`REQ-C10-S05`・`REQ-C12` の
  6要求と、実装単位 `F-21`・`B-16` が新設された。第6章は `F-21`・`B-16` の行を持たないため、
  **この2件の初期リリース判定だけは、照合ではなく新しい判断が要る。**
  また `REQ-C07-S02`・`REQ-C07-S03` が `F-17` へ、`REQ-C04-S03`・`REQ-C09-S05` が `F-20`・`B-15` へ
  加わったため、第6章の「最小形のみ」という判定で足りるかの確認も要る。

  ⚠️ **2026年9月11日の実測では、第8章の2つの一覧に現れない実装単位は21件だった。**
  `F-07`・`F-08`・`F-09`・`F-12`・`F-13`・`F-14`・`F-16`・`F-21`、
  `B-07`・`B-08`・`B-09`・`B-10`・`B-11`・`B-16`、`N-01`〜`N-07` である。

  ⚠️ **同種のずれが既に3件見つかっている。** 2026年9月7日、第10.1節の未決事項5件のうち
  認証・LLMプロバイダ・実行基盤の3件が `product-design.md` 第4.2節で既に決着しており、
  **正本の側だけが古いまま残っていた。** 対応表でも同じことが起きている見込みがある。
