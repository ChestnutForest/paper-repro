...
- 版: **v0.4**（`REQ-C12` の実装単位 `F-21`・`B-16` を第6章へ追加）
- 前版: v0.3（画面設計を `arch-guide/` へ移管）
...
3. **プロジェクトはステートマシン。** Phase 0〜6 は承認ゲートで進む状態遷移。
   ...
   **Phase 0〜6 は確定した工程モデルである**（要件 v0.3.1 第1.1節、2026年9月2日確定）。工程名を UI の文言や
...
要件 v0.3.1 第9章の対応表33行（要求 → 実装単位）に対し、本書は**実装単位 → 設計箇所**を与える。
...
| F-21 再現スコープ定義書 | 2章 インテーク／実装・検証台（配置は画面編の設計で確定）、3.2 | 含む |
