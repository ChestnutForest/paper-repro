...
- 現在の進捗: `requirements.md` v0.3。工程モデルの確定（第4バッチ）と再現の妥当性のサブ要求2件（第5バッチ）を反映済み。次は主題2（利用者の状態）と主題3（貢献の記録）の5択
- 版: v0.21（複数の分析文書を横断する場合の手順を 3.3 として追加）

---

## 1. 目的

菊田遥平『原論文から解き明かす生成AI』技術評論社, 2025 の対象部分を、要件検討における一次資料として節単位で読み、
得られた示唆と現在の要件定義を比較する。そのうえで、変更が必要な要求項目について利用者が
選択肢から方針を決定し、すべての選択後に `requirements.md` へ反映する変更案を作成する。

最初のサイクルでは「はじめに」を対象として`REQ-C01`〜`REQ-C09`を選択した。
次の増分サイクルでは「1.2 論文を読み解く技術」だけを追加分析し、結果を
[`requirements-analysis/section-1.2-reading-techniques.md`](requirements-analysis/section-1.2-reading-techniques.md)
に記録する。
...
- 菊田遥平『原論文から解き明かす生成AI』技術評論社, 2025 のPDF版の「はじめに」
- 同PDFの「1.2 論文を読み解く技術」本文（PDFページ4〜18）と関連脚注（19〜21）
...
- PDFの目次、著者紹介、「1.2」以外の第1章以降の本文
- PDFページ22〜23の参考文献と、24ページ以降の「おわりに」
- PDF全体の一括OCR
- 要件比較に不要なソースコードの精読
...
| 第4段階 | 要件定義変更案の作成 | 全11件と具体化済みサブ要求12件を統合済み・利用者の承認待ち |
| 増分分析 | 「1.2」の範囲確認、比較、要求候補の分類 | 分析完了（`REQ-C10`・`REQ-C11`とサブ要求候補10件） |
| 小節別詳細 | 「1.2.1」の境界、判定理由、要求文、受入基準の検証 | 完了（既存候補5件を具体化・新規IDなし） |
| 小節別増分 | 「1.2.1.1」の入手・分類・公開状態・信頼性参考情報の検証 | 完了（サブ要求2件を具体化・新規IDなし） |
| 小節別増分 | 「1.2.1.2」の電子読解・注釈・再参照・バックアップの検証 | 完了（`REQ-C09-S01`を具体化・新規IDなし） |
| 小節別増分 | 「1.2.1.3」の主張区分・直接支持範囲・誤り・誇張・バイアス・不確実性の検証 | 完了（`REQ-C10`の根拠を補強、`REQ-C10-S04`を具体化・新規IDなし） |
| 小節別増分 | 「1.2.2」の成立条件・実験設定・具体例・反例・実装読解・重要文献・アウトプットの検証 | 完了（既存サブ要求8件を具体化・新規IDなし） |
| 小節別増分 | 「1.2.2.1」の前提・帰結・成立範囲・読解深度・実験条件・自力検証 | 完了（`REQ-C10-S01`〜`S03`を詳細化・新規IDなし） |
| 小節別増分 | 「1.2.2.2」の操作可能な具体例・評価指標・支持例・反例・境界条件・仮説 | 完了（`REQ-C04-S01`・`REQ-C10-S03`を詳細化・新規IDなし） |
| 小節別増分 | 「1.2.2.3」の論文・公式実装対応、差異、版、入手可能性、検査可能性 | 完了（`REQ-C05-S01`を詳細化・新規IDなし） |
| 小節別増分 | 「1.2.2.4」の重要文献・データセット・関連実装の優先付け、深掘り、探索状態 | 完了（`REQ-C09-S02`を詳細化・新規IDなし） |
| 小節別増分 | 「1.2.2.5」の自己説明・未理解点の保持・公開範囲・秘匿判断の検証 | 完了（`REQ-C04-S02`・`REQ-C09-S03`を詳細化・新規IDなし） |
| 追加選択 | `REQ-C10`・`REQ-C11`の5択と変更案の更新 | 完了（第3バッチ・2/2件選択済み） |
| 変更案承認 | 全11件と小節別サブ要求候補を統合した変更案 | 利用者の承認・修正・保留待ち |
| 骨組み更新 | ID体系を`REQ-Cxx`へ一本化し、Phase 0〜6を暫定と明記 | 完了（`requirements.md` v0.2-draft、要求文本体は未統合） |
| 要求文統合 | 第11章の要求文と第12章の受入基準を`requirements.md`第3章へ統合 | 完了（`requirements.md` **v0.2**。メイン11件・サブ12件、実装単位F/B/Nと双方向対応） |

### 3.2 追加一次資料の増分サイクル

最初の変更案を作成した後に新しい一次資料を追加する場合は、変更案を直ちに確定せず、次の順序で処理する。

1. 新しい対象節の開始・終了境界を確定する。
2. 対象節だけを抽出し、ページ画像で境界を照合する。
3. 選択済み要求と変更案に対応付ける。
4. メイン要求、既存要求のサブ要求、要求にしない記述へ分類する。
5. 新しいメイン要求候補だけを5択へ進める。
6. 選択後に変更案を更新し、改めて承認を求める。

分析文書は[`requirements-analysis/`](requirements-analysis/)へ、選択結果は
[`requirements-decisions/`](requirements-decisions/)へ分離して保存する。

### 3.3 複数の分析文書を横断する場合（2026年9月2日追加）

⚠️ **一次資料が増え、分析文書が複数になった場合は 3.2 の手順を次のとおり読み替える。**

分析文書は一次資料ごとに独立して書かれるため、**同じ要求が別々の暫定IDで重複する。**
この状態で変更案へ統合すると、重複したまま採番することになる。

| 段階 | 担当する文書 | 役割 |
|---|---|---|
| 分析 | `requirements-analysis/` の各分析文書 | 一次資料ごとの要求候補の抽出 |
| **突合** | 同ディレクトリの `crosswalk-*.md` | **候補間の重複を統合し、現行要求への吸収を領域ごとに判定する** |
| 選択 | `requirements-decisions/batch-*.md` | 5択と、選択結果・理由・影響範囲の記録 |
| 反映 | `requirements.md` | 正本の更新 |

**このとき変更案（[`requirements-change-proposal.md`](requirements-change-proposal.md)）は作成しない。**
変更案が担っていた3つの役割のうち、**矛盾の検出は突合表が、トレーサビリティの確保は突合表と5択が**代替する。
残る「承認の単位」は、**5択の文書を単位とする**（下記）。

#### 承認の単位

- **5択の文書1件が承認の単位である。** 選択結果と理由をその文書へ記録した時点で承認とみなす。
- 8.1節の「4〜6項目を1バッチ」は、**主題が単独の場合は1件でもよい。**
  ただし候補間に接続がある場合は、接続するものをまとめて1つの選択にする。

#### ⚠️ 正本の変更と下流の追随を分ける

9章481行の「別の作業単位で」は、変更案の有無にかかわらず守る。

1. **`requirements.md` の更新**を1つのコミットにする。
2. `product-design.md`・`roadmap.md`・画面設計などの追随を、**別のコミット**にする。

正本と下流を同じコミットに混ぜると、正本の変更だけを差し戻すことができなくなる。
...
その後に追加した「1.2 論文を読み解く技術」の分析では、メイン要求候補`REQ-C10`・`REQ-C11`と、
既存要求を具体化するサブ要求候補10件が見つかった。根拠、要求文案、受入基準は
[`requirements-analysis/section-1.2-reading-techniques.md`](requirements-analysis/section-1.2-reading-techniques.md)
を根拠資料とする。`REQ-C10`・`REQ-C11`は第3バッチで選択肢4を選択済みであり、
選択結果と受入基準は決定台帳を正本とする。

「1.2.1」の小節別詳細は
[`requirements-analysis/section-1.2.1-reading-environment.md`](requirements-analysis/section-1.2.1-reading-environment.md)
に記録する。この詳細検証では新しい要求IDを追加せず、上位分析の`REQ-C10`、
`REQ-C03-S01`、`REQ-C07-S01`、`REQ-C09-S01`、`REQ-C10-S04`を具体化する。

`1.2.1.1`の限定分析は
[`requirements-analysis/section-1.2.1.1-paper-acquisition.md`](requirements-analysis/section-1.2.1.1-paper-acquisition.md)
に記録する。新しいメイン要求は追加せず、`REQ-C03-S01`と`REQ-C07-S01`を具体化する。

`1.2.1.2`の限定分析は
[`requirements-analysis/section-1.2.1.2-electronic-reading.md`](requirements-analysis/section-1.2.1.2-electronic-reading.md)
に記録する。新しいメイン要求は追加せず、`REQ-C09-S01`を要求文と受入基準まで具体化する。

`1.2.1.3`の限定分析は
[`requirements-analysis/section-1.2.1.3-human-authorship.md`](requirements-analysis/section-1.2.1.3-human-authorship.md)
に記録する。新しいメイン要求は追加せず、選択済みの`REQ-C10:4`の根拠を補強し、
`REQ-C10-S04`を要求文と受入基準まで具体化する。論文の真偽や不正をLLMだけで確定する要求は追加しない。

`1.2.2`の限定分析は
[`requirements-analysis/section-1.2.2-independent-reading-techniques.md`](requirements-analysis/section-1.2.2-independent-reading-techniques.md)
に記録する。新しいメイン要求は追加せず、`REQ-C04-S01`、`REQ-C04-S02`、`REQ-C05-S01`、
`REQ-C09-S02`、`REQ-C09-S03`、`REQ-C10-S01`〜`S03`を要求文と受入基準まで具体化する。

`1.2.2.1`の限定分析は
[`requirements-analysis/section-1.2.2.1-discussion-conditions.md`](requirements-analysis/section-1.2.2.1-discussion-conditions.md)
に記録する。新しいメイン要求は追加せず、`REQ-C10-S01`〜`S03`を対象小節の根拠に限定して
詳細化する。`REQ-C07-S01`と`REQ-C10-S04`の根拠は補強するが、論文の真偽、不正、
チェリーピックをLLMだけで確定する要求は追加しない。

`1.2.2.2`の限定分析は
[`requirements-analysis/section-1.2.2.2-concrete-examples.md`](requirements-analysis/section-1.2.2.2-concrete-examples.md)
に記録する。新しいメイン要求は追加せず、`REQ-C04-S01`と`REQ-C10-S03`を対象小節の根拠に限定して
詳細化する。`REQ-C10-S01`の根拠は補強するが、少数例による一般化、適用範囲外と反証の混同、
仮説の自動昇格を要求しない。

`1.2.2.3`の限定分析は
[`requirements-analysis/section-1.2.2.3-implementation-reading.md`](requirements-analysis/section-1.2.2.3-implementation-reading.md)
に記録する。新しいメイン要求は追加せず、`REQ-C05-S01`を対象小節の根拠に限定して詳細化する。
`REQ-C07:4`、`REQ-C08:4`、`REQ-C10-S04`の根拠は補強するが、論文または実装の一律優先、
不足情報の自動推定、非公開理由や研究不正の自動判定を要求しない。

`1.2.2.4`の限定分析は
[`requirements-analysis/section-1.2.2.4-important-references.md`](requirements-analysis/section-1.2.2.4-important-references.md)
に記録する。新しいメイン要求は追加せず、`REQ-C09-S02`を対象小節の根拠に限定して詳細化する。
`REQ-C03-S01`、`REQ-C04-S01`、`REQ-C07-S01`、`REQ-C08:4`の根拠は補強するが、引用数、推薦、
人気、利用実績の単一指標による自動採用、有償・非公開資料の代理取得を要求しない。
...
`REQ-C10`・`REQ-C11`の5つの選択肢、共通制約、選択結果、受入基準および段階開発条件は、
[`requirements-decisions/batch-03-options.md`](requirements-decisions/batch-03-options.md)
に記録する。両要求とも選択肢4を最終目標とし、`REQ-C11`は
選択肢1、2、3、4の順に段階開発する。
...
全9件を統合した変更内容は、
[`requirements-change-proposal.md`](requirements-change-proposal.md) を正本とする。ただし、「1.2」から得られた
追加候補については、選択結果を
[`requirements-decisions/batch-03-options.md`](requirements-decisions/batch-03-options.md)、
根拠を[`requirements-analysis/section-1.2-reading-techniques.md`](requirements-analysis/section-1.2-reading-techniques.md)
と各小節別分析へ分離して記録する。
...
その後の「1.2」増分分析で見つかった`REQ-C10`・`REQ-C11`も選択を完了し、
「1.2.1.1」〜「1.2.1.3」の限定分析を含めて変更案を再更新した。現在は利用者の承認、修正または保留の判断を待つ。
変更案が承認されるまで、確定要件、関連設計、ロードマップ、製品コードを変更しない。
...
- `requirements.md` の更新は、変更案の承認後にのみ行われる（**3.3 の増分サイクルでは5択の記録後**）。
- 追加した一次資料について、対象範囲と対象外が記録されている。
- 追加分析のメイン要求候補とサブ要求候補が区別されている。
- `REQ-C10`・`REQ-C11`の選択結果と、具体化した`REQ-C03-S01`、`REQ-C04-S01`〜`S02`、
  `REQ-C05-S01`、`REQ-C07-S01`、`REQ-C09-S01`〜`S03`、`REQ-C10-S01`〜`S04`が
  変更案へ反映されている。
- 「1.2.2.1」の限定分析で、`REQ-C10-S01`〜`S03`の要求文、受入基準、禁止する自動判定が
  詳細検証されている。
- 「1.2.2.2」の限定分析で、`REQ-C04-S01`・`REQ-C10-S03`の要求文、受入基準、
  禁止する自動一般化・自動昇格が詳細検証されている。
- 「1.2.2.3」の限定分析で、`REQ-C05-S01`の要求文、受入基準、版・入手・検査状態、
  禁止する一律優先・自動推定が詳細検証されている。
- 「1.2.2.4」の限定分析で、`REQ-C09-S02`の要求文、受入基準、時点付き参考信号、探索状態、
  禁止する単一指標による自動採用・代理取得が詳細検証されている。
...
- 利用者が変更案を明示的に承認するまで、確定要件として扱われない。
