...
このPowerShell出力はSHA照合とURL組み立てのための入力であり、利用者向けの最終成果物ではない。
エージェントの最終応答では、出力されたURLを1件ずつ独立した`text`コードブロックへ転記する。
ターミナル出力だけを示して「このコマンドがURL一覧を出す」と案内するだけでは完了にしない。
ファイル数による省略をしない。
...
ユーザーがVS CodeやPowerShellではなくチャットでURL一覧を求める場合も、出力形式は同じである。

1. URLごとに短い見出しを書く。
2. 直後に独立した`text`コードブロックを1つ置く。
3. コードブロック内にはURLを1件だけ、装飾なしで書く。
4. 次のURLは別の見出しと別のコードブロックへ置く。
5. リポジトリ、コミット、ブランチ、全ファイル、履歴の順を守る。
...
```text
https://github.com/ChestnutForest/paper-repro/commit/<実在する完全SHA>
```
````

`<実在する完全SHA>`は説明用表記であり、実際の出力では検証済みSHAへ置き換える。
コードブロックを1つにまとめたり、Markdownリンクだけで済ませたりしない。
